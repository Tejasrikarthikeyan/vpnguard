...
public class ReportService : IReportService
{
    ...
    {
        // Assemble current active state if available
        var currentData = await AssembleActiveReportDataAsync();
    ...
    private async Task<AnalysisReportData> AssembleActiveReportDataAsync()
    {
        ...
        if (_aiAnalysisService != null)
        {
            ...
            {
                data.HasAiAnalysis = true;
                data.AiTrafficType = ai.TrafficType;
                data.AiConfidence = ai.Confidence;
                data.AiPrediction = ai.Prediction;
                data.AnomalyDetected = ai.HasAnomalies;
                data.AiExplanation = ai.Explanation;
                data.TopFeatures = ai.TopFeatures.ToList();
                data.AiFeatures = ai.Features.ToDictionary(k => k.Key, v => v.Value);
                data.AiAnomalies = ai.Anomalies.ToList();
            }
        }
